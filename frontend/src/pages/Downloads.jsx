...
import { api, asArray, resolveImageUrl } from "../lib/api";
...
export default function Downloads() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12" data-testid="downloads-page">
      ...
      {loading ? (
        <div className="mt-8 grid md:grid-cols-2 gap-4 animate-pulse">
          {[...Array(4)].map((_, idx) => (
            <div key={idx} className="h-28 bg-muted rounded-2xl" />
          ))}
        </div>
      ) : (
        <div className="mt-8 grid md:grid-cols-2 gap-4">
          {items.map((d) => {
            const href = resolveImageUrl(d.file_url);
            return (
