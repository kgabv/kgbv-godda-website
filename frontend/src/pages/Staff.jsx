...
export default function Staff() {
  ...
  useEffect(() => { api.get("/teachers").then(r => setItems(asArray(r.data))).catch(() => setItems([])); }, []);
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {list.map(t => (
          <Card key={t.id} className="p-4 rounded-2xl text-center hover:shadow-xl smooth-color">
            {t.image_url ? (
              <img src={t.image_url} alt={t.name} className="h-32 w-32 mx-auto rounded-full object-cover ring-2 ring-primary/20"/>
            ) : (
